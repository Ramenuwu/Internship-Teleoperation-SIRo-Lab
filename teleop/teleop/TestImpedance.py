##THIS IS IMPEDANCE WITH NO F_EXT; FOR USAGE WITH FLEXIBLE JOINTS. tiago MIGHT have this shit
...
class TiagoImpedance(Node):
    ...
    def control_timer_callback(self):
        ...
        try:
            ...
            # ── 2. Cartesian mass matrix Λ(q) — now 6×6 ─────────────────────────────
            ...
            # ── 3. Gains — separate tuning for translation and rotation ──────────────
            #   Start with low rotational stiffness until translation is stable
            Kp = np.diag([2000.0, 2000.0, 2000.0,    # translational [N/m]
                           50.0,  50.0,  50.0])    # rotational    [Nm/rad]

            xi = 0.7   # damping ratio (same for all axes to start)

            # Factorization damping design — diagonal approximation of eq. 27
            Lambda_sqrt = np.diag(np.sqrt(np.diag(Lambda)))
            Kp_sqrt     = np.diag(np.sqrt(np.diag(Kp)))
            Kd          = 2.0 * xi * (Lambda_sqrt @ Kp_sqrt)               # (6 × 6)

            # ── 4. Errors ────────────────────────────────────────────────────────────
            # Position error
            e_pos = self.x_curr - self.x_des                               # (3,)

            # Orientation error via rotation matrix log map
            #   R_err = R_curr @ R_des.T  →  identity when aligned
            #   log3(R_err)  →  zero vector when aligned
            #   Sign convention: positive error → F pushes back toward R_des
            R_err = R_curr @ self.R_des.T
            e_ori = pin.log3(R_err)                                        # (3,)

            error_6 = np.concatenate([e_pos, e_ori])                       # (6,)

            # 6D end-effector velocity [v_linear; omega]
            v_6   = J6_arm @ self.dq                                       # (6,)

            # ── 5. Task-space force law — full 6D (paper eq. 19) ────────────────────
            #
            #   F_τ = −Kd·ė_x − Kp·e_x − Λ(q)·J̇q̇
            #          (ẍ_d = 0 since target is constant)
            #
            F_tau    = (- Kd     @ v_6
                        - Kp     @ error_6
            ...
            tau_des_arm = tau_task + tau_null + nle_arm * 0.465
