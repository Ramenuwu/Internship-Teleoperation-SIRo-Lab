...
class GravComp(Node):
    def __init__(self):
        super().__init__("left_gravcomp")

        self.effort_left_pub = self.create_publisher(
            Float64MultiArray,
            '/arm_left_effort_controller/commands',
            10
        )
    ...
    def joint_state_cb(self, msg: JointState):
        ...
        gravcomp_tau_arm = gravity_full[self.arm_vel_idx] * 0.4725
        total_tau = gravcomp_tau_arm + self.additional_efforts

        #print(total_tau)
        self.effort_left_pub.publish(Float64MultiArray(data=total_tau.tolist()))    
...
def main():
    ...
    try:
        ...
        node.effort_left_pub.publish(Float64MultiArray(data=tau_zeros.tolist()))
        print("Shutting down...")
    finally:
        rclpy.shutdown()
